...
from src.dags.connector import PgConnect
from psycopg.rows import class_row
from pydantic import BaseModel

sys.path.append(os.path.realpath("../"))
from dags.scripts import json2str
from dags.dds.srv_table_update import StgEtlSettingsRepository
...
class StgDataLoader:
    ...
    def get_deliveries(
        self, conn: PgConnect, threshold: int, limit: int
    ) -> List[Delivery]:
        with conn.cursor(row_factory=class_row(Delivery)) as cur:
            cur.execute(
                """
                   ...
                   object_value::JSON->>'tip_sum' as tip_sum FROM stg.deliverysystem_deliveries
                   )
        ...
        with conn.cursor(row_factory=class_row(Courier)) as cur:
            cur.execute(
                """
                   ...
                   FROM stg.deliverysystem_couriers WHERE id > %(threshold)s ORDER BY id ASC LIMIT %(limit)s;
                """,
                {"threshold": threshold, "limit": limit},
            )
            obj = cur.fetchall()
    ...
    def insert_deliveries(self, conn, delivery: Delivery) -> None:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """
                    INSERT INTO dds.dm_deliveries(delivery_id, order_id, courier_id, address, delivery_ts, rate, sum, tip_sum)
        ...
        with self.stg.connection() as conn:
            ...
            if not wf_setting:
                wf_setting = EtlSetting(
                    ...
                    workflow_settings={"last_loaded_offset": -1},
                )
            last_loaded_offset = wf_setting.workflow_settings["last_loaded_offset"]
            print("Last loaded offset {}".format(last_loaded_offset))
            load_queue = self.get_couriers(
                conn, threshold=last_loaded_offset, limit=self.batch
            )
            if len(load_queue) == 0:
                print("Have not data")
            else:
            ...
            wf_setting.workflow_settings["last_loaded_offset"] = max(
    ...
    def load_deliveries(self):
        with self.stg.connection() as conn:
            ...
            last_loaded_offset = wf_setting.workflow_settings["last_loaded_offset"]
            print("Last loaded offset {}".format(last_loaded_offset))
            load_queue = self.get_deliveries(
                conn, threshold=last_loaded_offset, limit=self.batch
            )
            if len(load_queue) == 0:
                print("Have not data")
                ...
                for load_data in load_queue:
                    ...
                    delivery = Delivery(
                        ...
                        courier_id=load_data.courier_id,
                        address=load_data.address,
                        delivery_ts=load_data.delivery_ts,
                        rate=load_data.rate,
                        sum=load_data.sum,
                        tip_sum=load_data.tip_sum,
                    )
